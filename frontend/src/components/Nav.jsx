...
const Nav = () => {
  ...
  return (
    ...
    {showSearch && userData.role=="user" && <div className='w-[90%] h-[70px] bg-white shadow-xl rounded-lg items-center gap-[20px] flex fixed top-[80px] left-[5%] md:hidden'>
      ...
      <h1 className='text-3xl font-bold mb-2 text-[#ff4d2d]'>MealDrop</h1>
      {userData.role=="user" &&
     ...
     {userData.role=="user" && 
     (showSearch ? <RxCross2 size={25} className='text-[#ff4d2d] md:hidden cursor-pointer' onClick={()=>setShowSearch(false)} /> :
     ...
     {myShopData && 
      ...
      <button onClick={()=>navigate(`/add-item`)} className='flex md:hidden items-center p-2 cursor-pointer rounded-full bg-[#ff4d2d]/10 text-[#ff4d2d]'>
        <FaPlus size={20} />
      </button></>}
      ...
      <div onClick={()=>setShowInfo(prev=>!prev)} className='w-[40px] h-[40px] rounded-full bg-[#ff4d2d] text-white flex items-center justify-center text-[18px] shadow-xl font-semibold cursor-pointer'>
