...
export const acceptOrder = async (req,res) => {
  try {
    ...
    if(alreadyAssigned){
      return res.status(404).json({message:"You are already assigned to another order"})
    }
...
export const sendDeliveryOtp = async (req,res) => {
  try {
    ...
    return res.status(500).json({ message: `Send deliveryOtp error ${error}` });
  }
}
...
export const verifyDeliveryOtp = async (req,res) => {
  try {
    ...
    return res.status(500).json({ message: `Verify deliveryOtp error ${error}` });
  }
}
